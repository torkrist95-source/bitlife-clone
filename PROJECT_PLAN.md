...
- **Name** — a text field with a 🎲 randomize option (reuses the same name pool as random generation); typing nothing and hitting the dice is just as valid as typing a name by hand
...
- A **🎲 Randomize Character** option rolls name, gender, and country together in one tap — the player can still edit any of the results afterward before starting
- Everything else — starting stats, personality traits, Looks, family — stays randomized, keeping the "roll a life and see what you get" spirit rather than turning this into a full builder
...
- **Save/load with multiple lives**: `localStorage` holds a `{ saveVersion, activeLifeId, lives: { [id]: { id, createdAt, updatedAt, character } } }` container — several independent playable lives can exist at once, each fully preserved when the player switches away from it. Autosaves after character creation, every Age Up, and every event choice, plus a manual Save button with a toast confirmation. A **Home / My Lives** screen (reached via Settings → My Lives, not the bottom nav — never shown automatically) lists every saved life as a card with Continue and Delete, marks the active one, and hosts Create New Life. Deleting the active life while others remain returns to My Lives so the player can pick one; deleting the last remaining life goes straight to Character Creation instead of showing an empty My Lives screen. An older single-character (v1) save format is transparently migrated into the multi-life shape on first load, so no existing progress was lost when this shipped. Missing fields are defaulted on load rather than crashing, and corrupted save data is quarantined (not destroyed).
- **Startup routing**: a first-time player (no saved lives at all) goes straight to Character Creation, never to My Lives. A returning player with an active life goes straight into gameplay, skipping My Lives entirely. My Lives only appears automatically in the one edge case where saved lives exist but none is currently active (e.g. the active life was deleted last session) — every other path to it is manual, via Settings.
